...
class TestMainModuleInterface:
    ...
    def test_all_exports_available(self):
        ...
        for export in expected_exports:
            assert hasattr(sys.modules['typing_compat'], export), f"Missing export: {export}"
    ...
    def test_import_star_works(self):
        ...
        for item in all_items:
            assert hasattr(sys.modules['typing_compat'], item), f"Item in __all__ not available: {item}"
...
class TestUtilityFunctions:
    ...
    def test_is_list_like_function(self):
        ...
        assert is_list_like(List[int]) is True
        
        if PY39_PLUS:
            # On Python 3.9+, might use built-in list
            list_str = list[str] if hasattr(list, '__class_getitem__') else List[str]
            assert is_list_like(list_str) is True
    ...
    def test_is_dict_like_function(self):
        ...
        assert is_dict_like(Dict[str, int]) is True
        
        if PY39_PLUS:
            # On Python 3.9+, might use built-in dict
            dict_type = dict[str, int] if hasattr(dict, '__class_getitem__') else Dict[str, int]
            assert is_dict_like(dict_type) is True
